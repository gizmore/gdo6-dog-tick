    ...
    public function dogExecute(DOG_Message $message, DOG_User $user)
    {
        ...
        elseif ($tick = DOG_Tick::tickFor($user))
        {
            $message->rply('err_dog_already_ticked', [
                $user->displayFullName(), $tick->getTicker()->displayFullName(), $tick->displayDate()]);
        }
        
        else
        {
            $tick = DOG_Tick::tick($user, $message->user);
            $message->rply('msg_dog_ticked', [$user->displayFullName(), DOG_Tick::numTicks($message->user)]);
            $numTicks = DOG_Tick::numTicks($message->user);
            $user->send($message->t('msg_dog_you_are_infected', [$message->user->displayFullName(), $numTicks]));
        }
    }
