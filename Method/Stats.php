    ...
    public function dogExecute(DOG_Message $message, $section, $page)
    {
        if ($section === 'total')
        {
            ...
            return $message->rply('msg_dog_tickstats_total', [
                DOG_Tick::totalVictims(), $bestPlayer->displayFullName(), DOG_Tick::numTicks($bestPlayer)]);
        }
        elseif ($section === 'top10')
        {
            $nItems = DOG_Tick::table()->select('COUNT(DISTINCT(tick_by))')->exec()->fetchValue();
            if ($nItems == 0)
            {
                return $message->rply('err_no_data');
            }
            ...
            if ($page > $nPages)
            {
                return $message->rply('err_page', [$nPages]);
            }
            ...
            $message->rply('msg_dog_tickstats_top10', [$nItems, $page, $nPages, $back]);
        }
        elseif ($section === 'victims')
        {
            $nItems = DOG_Tick::table()->select('COUNT(DISTINCT(tick_to))')->exec()->fetchValue();
            if ($nItems == 0)
            {
                return $message->rply('err_no_data');
            }
            ...
            $message->rply('msg_dog_tickstats_victims', [$nItems, $page, $nPages, $back]);
        }
    }
